...
def train_model(model, train_loader, val_loader, criterion, optimizer, num_epochs=10):
    ...
    for epoch in range(num_epochs):
        ...
        save_path = "activity_cnn_model_multilabel_10.pth"
...
'''
  def forward(self, x):
        ...
        x = self.extra_conv(x)
        print(f"After Extra Conv: {x.shape}")
        x = self.model.conv1(x)
        print(f"After ResNet Conv1: {x.shape}")
        x = self.model.bn1(x)
        ...
        x = self.model.fc(x)
        print(f"After Fully Connected (Output): {x.shape}")
        return x
'''
