...
class EquipmentActivityDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Convert each activity to tensor and pad if necessary
        activity_tensors = []
        for activity_str in activities:
            if activity_str in activity_mapping:
                activity_int = activity_mapping[activity_str]
                activity_tensors.append(activity_int)
            else:
                #raise ValueError(f"Unknown activity label '{activity_str}'")
                print(f"Unknown activity label '{activity_str}'")  # Debug print for unknown activities

        #print("Activity Tensors:", activity_tensors)  # To debug if the tensor is empty or not


        # Pad the activity tensor to ensure uniform length (e.g., maximum 5 labels)
        max_activities = 5
        padded_activities = torch.zeros(max_activities, dtype=torch.float32)  # Use float32 for BCEWithLogitsLoss
        #print(padded_activities)
        for i, activity in enumerate(activity_tensors[:max_activities]):  # Limit to max activities
            padded_activities[i] = activity

        print("Padded Activities:", padded_activities)  # Check padded tensor
        return image, padded_activities
...
def train_model(model, train_loader, val_loader, criterion, optimizer, num_epochs=50):
    ...
    for epoch in range(num_epochs):
        ...
        save_path = "activity_cnn_model_multilabel.pth"
