...
color_map = plt.cm.get_cmap('Dark2', len(activity_labels))  # Use a color map with sufficient colors

# Create a plot for each vehicle in a subplot
fig, axes = plt.subplots(1, len(vehicles), figsize=(15, 1.5 * len(vehicles)))
if len(vehicles) == 1:
...
for i, vehicle in enumerate(vehicles):
    ...
    scatter = axes[i].scatter(frames, numeric_activities, c=numeric_activities, cmap=color_map, s=60, alpha=0.7)

    axes[i].set_yticks(range(len(activity_labels)))
    axes[i].set_yticklabels(activity_labels)
    axes[i].set_xlabel('Frame Number', fontweight='bold')  # Bold the label
    axes[i].set_ylabel('Activity', fontweight='bold')  # Bold the label
    axes[i].set_title(f'{vehicle} Activity Over Time', fontweight='bold')  # Bold the title
    axes[i].grid(True)

    # Add a legend
    handles, labels = scatter.legend_elements()
    axes[i].legend(handles, activity_labels, title="Activities", loc="best", fontsize=10, title_fontsize=12, labelspacing=1.2)

# Adjust the layout for even spacing and show the plot
plt.tight_layout()
plt.subplots_adjust(hspace=0.3, wspace=0.4)  # Adjust horizontal and vertical spacing
plt.show()
